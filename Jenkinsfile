  ...
  stages {
    ...
    stage('Test') {
     ...
     stage('Deploy Development') {
      ...
      steps {
            sh 'mvn -X -U -V -e -B -gs $M2SETTINGS -DskipTests deploy -DmuleDeploy -Dmule.version=$MULE_VERSION -Danypoint.username=$DEPLOY_CREDS_USR -Danypoint.password=$DEPLOY_CRED_PSW -Dcloudhub.app=$APP_NAME -Dcloudhub.environment=$ENVIRONMENT -Dcloudhub.bg=$BG -Dcloudhub.worker=$WORKER'
      }
    }
